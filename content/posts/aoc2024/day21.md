...
```
That's an `n`, for nanoseconds, not microseconds like most solution so far, it is 45000x faster than the original version, even ignoring the fact `hyperfine` often overestimates.
